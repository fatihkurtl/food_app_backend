<div>
    ...
    <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md mb-8">
        ...
        <form wire:submit.prevent="addCategory" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                <label for="photo" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Kategori
                    Fotoğrafı</label>
                ...
                @if ($photo)
                    <div class="mt-4">
                        <img src="{{ $photo->temporaryUrl() }}" alt="Fotoğraf Önizlemesi" class="w-32 h-32 object-cover rounded-md">
                    </div>
                @endif
            </div>
        ...
        @foreach ($categories as $category)
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
                ...
                <div class="mt-4 flex justify-between">
                    <a href="{{ route('admin.category.recipes', $category->id) }}" class="text-blue-500 hover:underline">Düzenle</a>
                    <button type="button" class="text-red-500 hover:underline"
                        wire:click="deleteCategory({{ $category->name }})">Sil</button>
                </div>
            </div>
        @endforeach
    </div>
