<div>
    ...
    <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md mb-8">
        ...
        <form wire:submit.prevent="editCategory" enctype="multipart/form-data" class="space-y-4">
            <div>
                <label for="edited_category_name" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Kategori
                    Adı</label>
            ...
            <div>
                ...
                @error('photo')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
                @if ($edited_photo && is_string($edited_photo))
                    <div class="mt-4">
                        <img src="{{ asset('storage/' . $edited_photo) }}" alt="Fotoğraf Önizlemesi" class="w-32 h-32 object-cover rounded-md">
                    </div>
                 @elseif ($edited_photo)
                    <div class="mt-4">
                        <img src="{{ $edited_photo->temporaryUrl() }}" alt="Mevcut Fotoğraf" class="w-32 h-32 object-cover rounded-md">
                    </div>
                @endif
            </div>
